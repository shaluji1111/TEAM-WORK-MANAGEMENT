...
import { CheckCheck, ListTodo, LayoutGrid, Repeat2, UsersRound, Settings2, LogOut, Clock3, ChevronRight, CalendarDays, LoaderCircle, type LucideIcon } from "lucide-react";
...
function NavigationIcon({ icon: Icon, size }: { icon: LucideIcon; size: number }) {
  const { pending } = useLinkStatus();
  return pending ? <LoaderCircle size={size} className="navigation-spinner" aria-label="Loading page" /> : <Icon size={size} />;
}
export function AppShell({ person, children, today }: { person: { name: string; role: Role; username: string }; children: React.ReactNode; today: string }) {
  ...
  useEffect(() => {
    ...
    function refresh() {
      const now = Date.now();
      // Returning to a tab can fire focus and visibilitychange together.
      if (now - lastRefresh < 1000 || document.visibilityState !== "visible" || document.querySelector('[role="dialog"]') || ["INPUT", "TEXTAREA", "SELECT"].includes(document.activeElement?.tagName || "")) return;
      lastRefresh = now;
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar"><Link href="/my-tasks" className="sidebar-brand"><span className="brand-symbol"><CheckCheck size={22} /></span>Team Task Tracker</Link>
      <p className="sidebar-eyebrow">WORKSPACE</p><nav className="nav-links" aria-label="Main navigation">{visible.map(link => <Link key={link.href} href={link.href} className={cn("nav-link", pathname.startsWith(link.href) && "active")} aria-current={pathname.startsWith(link.href) ? "page" : undefined}><NavigationIcon icon={link.icon} size={18} />{link.label}</Link>)}</nav>
    ...
    <div className="main-column"><header className="topbar"><div className="breadcrumb"><span>Workspace</span><ChevronRight size={14} /><strong>{title}</strong></div><div className="row topbar-controls"><span className="topbar-date">{today}</span><ThemeControl /><span className="avatar topbar-avatar" title={person.name}>{initials(person.name)}</span></div></header>
      <nav className="mobile-nav" aria-label="Mobile navigation">{visible.map(link => <Link key={link.href} href={link.href} className={pathname.startsWith(link.href) ? "active" : ""} aria-current={pathname.startsWith(link.href) ? "page" : undefined}><NavigationIcon icon={link.icon} size={15} />{link.label}</Link>)}<button className="signout" onClick={signout} aria-label="Sign out"><LogOut size={17} /></button></nav>
